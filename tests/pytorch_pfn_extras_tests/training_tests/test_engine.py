...
class TestEngine:
    def test_engine_extension(self):
        engine = ppe.training._trainer._Engine(
    ...
    def test_engine_load_state_dict(self):
        manager = ppe.training.ExtensionsManager(
            {}, {}, 10, iters_per_epoch=100)
        engine = ppe.training._trainer._Engine(None, {}, optimizers={}, max_epochs=1)
...
class TestEngineInvalid:
    def test_engine_wrong_models(self):
        with pytest.raises(ValueError, match='model must be an instance'):
            ppe.training._trainer._Engine(None, object(), optimizers={}, max_epochs=10)

    def test_engine_not_started(self):
        engine = ppe.training._trainer._Engine(None, {}, optimizers={}, max_epochs=10)
