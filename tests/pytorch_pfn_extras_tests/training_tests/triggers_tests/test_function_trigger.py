...
def test_function_is_called() -> None:
    ...
    args = [MagicMock()]
    kwargs = {"a": MagicMock()}
    trigger = FunctionTrigger(fn=fn, args=args, kwargs=kwargs, trigger=(1, "iteration"))
...
def test_trigger_with_value() -> None:
    ...
    args = [value]
    trigger = FunctionTrigger(fn=lambda x: x["value"], args=args, trigger=(1, "iteration"))
    manager = ExtensionsManager({}, {}, 1, iters_per_epoch=10)
    with manager.run_iteration():
        pass
...
@pytest.mark.parametrize(
    "trigger, iters_per_epoch", [((1, "iteration"), 10), ((1, "epoch"), 20), ((0.123, "epoch"), 17)]
)
def test_with_interval_trigger(trigger: TriggerLike, iters_per_epoch: int) -> None:
