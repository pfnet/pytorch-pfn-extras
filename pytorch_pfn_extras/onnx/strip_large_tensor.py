...
def _strip_raw_data(tensor):
    ...
    onnx.external_data_helper.set_external_data(
        tensor, location=json.dumps(meta_dict), length=len(tensor.raw_data)
    )
...
def _strip_large_tensor_tool_impl(
    onnx_path, out_onnx_path, large_tensor_threshold
):
...
def _strip_large_tensor_tool():
    ...
    _strip_large_tensor_tool_impl(
        args.onnx_path, out_onnx_path, args.large_tensor_threshold
    )
