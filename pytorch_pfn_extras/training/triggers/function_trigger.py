...
from typing import Callable, Optional, Sequence, Any, TYPE_CHECKING, Dict
...
class FunctionTrigger(trigger_module.Trigger):
    ...
    def __init__(self, fn: Callable[..., bool], args: Optional[Sequence[Any]] = None, kwargs: Optional[Sequence[Any]] = None, trigger: "TriggerLike" = (1, "iteration")) -> None:
        self._fn = fn
        self._args = args or []
    ...
    def may_fire(self, iteration: int, epoch_len: int) -> bool:
        if self._interval_trigger.may_fire(iteration=iteration, epoch_len=epoch_len):
            return self._fn(*self._args, **self._kwargs)
